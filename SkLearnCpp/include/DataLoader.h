...
public:
    ...
    bool load_data(const std::string& filename);   //loads data and returns true if successful
    void display_data(int cell_length=14) const;
};
...
template<typename T> void DataLoader<T>::display_data(int cell_length) const{
    std::cout<<std::left;
...
template<typename T> bool DataLoader<T>::load_data(const std::string& filename){
